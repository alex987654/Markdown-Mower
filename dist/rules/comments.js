...
export const removeHtmlComments = {
    ...
    category: "comments",
    severity: "medium",
    description: "Remove <!-- comments --> from the document. Invisible to readers but counted by tokenizers. Tooling-control comments are preserved.",
    run(text) {
        ...
        while (i < text.length) {
            if (text[i] === "<" &&
                ...
                const isToolingDirective = allowList.some((tag) => inner.startsWith(tag));
                ...
                saved += consumed;
                if (!sample)
                    sample = text.slice(i, Math.min(i + 60, end + 3));
                i = end + 3;
            ...
            diagnostics: [
                {
                    ...
                    detail: "Tooling directives (prettier-ignore, markdownlint-*, vale-*) are preserved.",
                    occurrences,
                    charsSaved: saved,
...
export function findCodeFenceRanges(text) {
    const ranges = [];
    const fenceRe = /^([ \t]*)(```+|~~~+)([^\n]*)$/gm;
    let m;
    let openFence = null;
