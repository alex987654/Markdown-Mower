...
export const normalizeSmartQuotes = {
    id: "MD-AI010",
    name: "Replace smart quotes",
    category: "punctuation",
    severity: "low",
    description: "Convert curly quotes (‘’ “”) to straight ASCII quotes. Saves bytes and tokenizes more predictably.",
    run(text) {
        let count = 0;
        let saved = 0;
        const out = text
...
export const normalizeDashes = {
    ...
    category: "punctuation",
    severity: "low",
    description: "Convert em-dash (—) and en-dash (–) to ASCII hyphen-minus (-). Aggressive: changes typographic feel. Off by default.",
    run(text, opts) {
        if (!opts.aggressive)
            return { text, diagnostics: [] };
        let count = 0;
