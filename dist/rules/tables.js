    ...
    while (i < lines.length - 1) {
        ...
        if (!isInFence(i) &&
            ...
            if (headerCells.length >= 2) {
                const rows = [];
                const rawLines = [headerLine, alignLine];
                let j = i + 2;
                while (j < lines.length) {
                    const row = lines[j];
                    if (!row.includes("|") || row.trim() === "")
                        break;
...
export const stripTablePadding = {
    id: "MD-AI050",
    name: "Strip table cell padding",
    category: "tables",
    severity: "medium",
    description: "Remove extraneous spaces inside table cells. GFM treats '| value |' and '|value|' identically; the padding is purely visual.",
    run(text) {
            ...
            diagnostics: [
                {
                    ...
                    detail: "GFM does not require padding spaces. Many doc generators emit padded cells for visual alignment in source; this is invisible in rendered output and pure overhead for an LLM.",
                    occurrences,
                    charsSaved: saved,
                    sampleBefore: sample
                        ? sample.length > 60
...
function compactAlignmentRow(align) {
...
export const twoColumnTableToList = {
    id: "MD-AI051",
    name: "Convert 2-column tables to lists (aggressive)",
    category: "tables",
    severity: "high",
    description: "Two-column key/value tables tokenize ~30-50% denser as '- Key: Value' lists. Changes rendered layout — off by default.",
    run(text, opts) {
        if (!opts.aggressive)
            return { text, diagnostics: [] };
        const tables = parseTables(text);
        ...
        for (let idx = tables.length - 1; idx >= 0; idx -= 1) {
            ...
            const isGenericHeader = /^(key|name|item|attribute|field|property)$/i.test(t.headerCells[0]) &&
            ...
            if (after.length < before.length) {
                ...
                saved += before.length - after.length;
                occurrences += 1;
                lines.splice(t.startLine, t.endLine - t.startLine + 1, ...listLines);
            }
        }
        if (occurrences === 0)
            return { text, diagnostics: [] };
            ...
            diagnostics: [
                {
                    ...
                    detail: "Aggressive change: visual rendering shifts from a table to a bullet list. Recommended only for AI-target documents.",
                    occurrences,
                    charsSaved: saved,
                    sampleBefore: sample,
                },
...
export const flagWideTables = {
    id: "MD-AI052",
    name: "Flag wide tables for review",
    category: "tables",
    severity: "medium",
    description: "Report tables with five or more columns. Wide tables often pay heavy padding/pipe overhead per cell and may be more efficient in a different format.",
    run(text) {
        const tables = parseTables(text);
            ...
            diagnostics: [
                {
                    ...
                    detail: "Wide tables tend to be where token cost concentrates. Options: trim columns; split into multiple narrower tables; or, for AI-target docs, consider an alternative representation (newline-separated records, JSON, or a denser 'TOON'-style format).",
