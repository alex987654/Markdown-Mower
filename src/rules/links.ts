...
export const convertRepeatedLinksToReferences: Rule = {
  id: "MD-AI060",
  name: "Reference-link conversion",
  category: "links",
  severity: "medium",
  description:
    "Convert inline links to reference style when the same URL appears more than once. Adds a definitions block at the end of the document.",
  run(text): RuleResult {
    ...
    const linkRe = /\[([^\[\]\n]+?)\]\((https?:\/\/[^\s)]+)\)/g;
    const occurrences: { match: string; index: number; text: string; url: string }[] = [];
    ...
    if (occurrences.length === 0) return { text, diagnostics: [] };

    // Group by URL.
    const byUrl = new Map<string, typeof occurrences>();
    ...
    // Decide which URLs to convert: appears 2+ times AND length > 15.
    type Conversion = {
      url: string;
      label: string;
    ...
    // but guard anyway).
    if (saved < 0) return { text, diagnostics: [] };
      ...
      diagnostics: [
        {
          ...
          detail:
            "Each URL appears once in a definitions block; inline links use a short numeric label. Break-even is around 15-char URLs used twice.",
...
export const stripUrlTrackingParameters: Rule = {
  id: "MD-AI061",
  name: "Strip URL tracking parameters",
  category: "links",
  severity: "low",
  description:
    "Remove utm_*, fbclid, gclid, mc_cid, mc_eid, _hsenc, _hsmi, igshid, and similar tracking parameters from URLs.",
  run(text): RuleResult {
    const fenceRanges = findCodeFenceRanges(text);
    const isInFence = (idx: number) =>
      fenceRanges.some((r) => idx >= r.start && idx < r.end);
...
export const flagBareLongUrls: Rule = {
  id: "MD-AI062",
  name: "Flag long bare URLs",
  category: "links",
  severity: "low",
  description:
    "Identify bare URLs longer than 50 characters that appear in prose. Consider wrapping them as [short text](url) or moving to references.",
  run(text): RuleResult {
    const fenceRanges = findCodeFenceRanges(text);
    const isInFence = (idx: number) =>
      fenceRanges.some((r) => idx >= r.start && idx < r.end);
      ...
      diagnostics: [
        {
          ...
          detail:
            "These render as clickable links but the full URL is part of the document. Consider giving them link text or moving them to a references section.",
          occurrences: count,
          charsSaved: 0,
