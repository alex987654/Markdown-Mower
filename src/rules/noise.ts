...
export const stripDecorativeBanners: Rule = {
  id: "MD-AI020",
  name: "Strip decorative banner lines",
  category: "noise",
  severity: "medium",
  description:
    "Remove lines that consist solely of repeated decorative characters (═══, ───, ▓▓▓, ✦✦✦) used as visual ornaments, not as Markdown horizontal rules.",
  run(text): RuleResult {
    ...
    for (const line of lines) {
      ...
      if (isOrnament) {
        ...
        if (!sampleBefore) sampleBefore = line.slice(0, 40);
        continue;
      }
      out.push(line);
    }
      ...
      diagnostics: [
        {
          ...
          detail:
            "These lines are visual ornaments that contribute no information to an LLM consumer. They tokenize as their full length.",
          occurrences,
          charsSaved: saved,
          sampleBefore,
          sampleAfter: "",
...
export const collapseHorizontalRules: Rule = {
  id: "MD-AI021",
  name: "Collapse adjacent horizontal rules",
  category: "noise",
  severity: "low",
  description:
    "Collapse two or more horizontal rules separated only by blank lines into a single rule.",
  run(text): RuleResult {
    ...
    for (const line of lines) {
      ...
      for (const b of pendingBlanks) out.push(b);
      pendingBlanks = [];
      out.push(line);
...
export const removeEmptyHeaders: Rule = {
  id: "MD-AI022",
  name: "Remove empty headings",
  category: "noise",
  severity: "medium",
  description:
    "Drop heading lines that have no text content (e.g. '##' or '### ' on its own).",
  run(text): RuleResult {
    const lines = text.split("\n");
    let occurrences = 0;
    let saved = 0;
...
export const stripDecorativeEmojiRows: Rule = {
  id: "MD-AI023",
  name: "Strip decorative emoji rows",
  category: "noise",
  severity: "medium",
  description:
    "Remove lines that consist solely of three or more emoji (with optional spacing). Inline emoji in prose are kept.",
  run(text): RuleResult {
    ...
    const emojiClass =
      "[\\u{1F300}-\\u{1FAFF}\\u{2600}-\\u{27BF}\\u{1F000}-\\u{1F02F}\\u{1F0A0}-\\u{1F0FF}\\u{2700}-\\u{27BF}\\u{FE0F}\\u{200D}]";
      ...
      diagnostics: [
        {
          ...
          detail:
            "Each emoji typically tokenizes as 2-4 tokens. A line of pure emoji adds attention noise without information for an LLM reader.",
          occurrences,
          charsSaved: saved,
          sampleBefore: sample || undefined,
        },
