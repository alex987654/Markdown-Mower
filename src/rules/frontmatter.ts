...
export const analyzeFrontmatter: Rule = {
  id: "MD-AI100",
  name: "Analyze frontmatter weight",
  category: "frontmatter",
  severity: "low",
  description:
    "Detect YAML (---) or TOML (+++) frontmatter and report its estimated token cost. Never auto-stripped.",
  run(text): RuleResult {
    const yamlMatch = /^---\n([\s\S]*?)\n---\n?/.exec(text);
    const tomlMatch = /^\+\+\+\n([\s\S]*?)\n\+\+\+\n?/.exec(text);
    const match = yamlMatch ?? tomlMatch;
    if (!match) return { text, diagnostics: [] };
    const block = match[0];
      ...
      diagnostics: [
        {
          ...
          detail:
            "Kept by default. If this document is being sent to an LLM that doesn't need the metadata (e.g. you're attaching a published .md to a chat), enable the aggressive 'Strip frontmatter' rule below.",
          occurrences: 1,
          charsSaved: 0,
...
export const stripFrontmatter: Rule = {
  id: "MD-AI101",
  name: "Strip frontmatter (aggressive)",
  category: "frontmatter",
  severity: "high",
  description:
    "Remove the YAML or TOML frontmatter block entirely. Off by default — only enable when the consumer doesn't need the metadata.",
      ...
      diagnostics: [
        {
          ...
          detail:
            "Aggressive: this removes potentially load-bearing metadata. Re-enable if your consumer needs it.",
          occurrences: 1,
          charsSaved: saved,
        },
      ],
