...
export const removeBom: Rule = {
  id: "MD-AI001",
  name: "Strip UTF-8 BOM",
  category: "whitespace",
  severity: "low",
  description:
    "Remove leading byte-order mark (U+FEFF). Editors sometimes add it; tokenizers always count it.",
  run(text): RuleResult {
    if (!text.startsWith("\uFEFF")) {
...
export const normalizeLineEndings: Rule = {
  id: "MD-AI002",
  ...
  run(text): RuleResult {
    ...
    if (total === 0) return { text, diagnostics: [] };
...
export const trimTrailingWhitespace: Rule = {
  ...
  severity: "low",
  description:
    "Remove trailing spaces and tabs at the end of lines. Two trailing spaces force a hard line break in some flavors — preserved when intentional.",
  run(text): RuleResult {
    ...
    const out = lines.map((line, i) => {
      ...
      const isHardBreak =
        trailing === "  " && next !== undefined && next.trim() !== "";
      if (isHardBreak) return line;
...
export const collapseBlankLines: Rule = {
  ...
  severity: "low",
  description:
    "Collapse runs of three or more blank lines into a single blank line. Preserves single and double blank lines, which carry structural meaning.",
  run(text): RuleResult {
    let occurrences = 0;
    let saved = 0;
...
export const normalizeInvisibleSpaces: Rule = {
  id: "MD-AI005",
  name: "Normalize invisible whitespace",
  category: "whitespace",
  severity: "low",
  description:
    "Convert non-breaking spaces, zero-width characters, and exotic whitespace to plain spaces or remove them entirely.",
  run(text): RuleResult {
    let occurrences = 0;
    let saved = 0;
