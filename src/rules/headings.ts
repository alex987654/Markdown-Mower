...
export const flagOrphanHeadings: Rule = {
  id: "MD-AI070",
  name: "Flag orphan headings",
  category: "headings",
  severity: "low",
  description:
    "Identify headings with no body content (only another heading directly follows). Often signals over-structured documents.",
  run(text): RuleResult {
      ...
      diagnostics: [
        {
          ...
          detail:
            "Often a sign of redundant structure. Consider whether the empty parent heading is needed or whether children can be promoted.",
          occurrences: count,
          charsSaved: 0,
...
// Info only.
export const flagRedundantHeadingText: Rule = {
  id: "MD-AI071",
  name: "Flag redundant heading text",
  category: "headings",
  severity: "low",
  description:
    "Identify headings with trailing punctuation (colons, periods) or repeated identical text. These are usually accidental.",
  run(text): RuleResult {
    const lines = text.split("\n");
    let count = 0;
    let sample = "";
    for (const line of lines) {
      ...
      if (trailingPunct || hasRepeat) {
        count += 1;
        if (!sample) sample = line.length > 60 ? line.slice(0, 60) + "..." : line;
      }
    }
    if (count === 0) return { text, diagnostics: [] };
    return {
